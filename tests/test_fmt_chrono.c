...
void fmt_chrono_default_f(void)
{
    ...
    EVAL(mstr_format("@{0:t:%f}@", &s, 1, &time_value));
...
void fmt_chrono_default_g(void)
{
    MString s;
    EVAL(mstr_create_empty(&s));
    EVAL(mstr_format("@{0:t:%g}@", &s, 1, &time_value));
...
void fmt_chrono_userdef_month(void)
{
    ...
    // 1位~2位
    EVAL(mstr_format(
        "@{0:t:%M}@{1:t:%M}@", &s, 2, &time_value_sing, &time_value_doub
    ));
...
void fmt_chrono_userdef_day(void)
{
    ...
    // 1位~2位
    EVAL(mstr_format(
        "@{0:t:%d}@{1:t:%d}@", &s, 2, &time_value_sing, &time_value_doub
    ));
    ASSERT_EQUAL_STRING(&s, "@1@10@");
    // 固定2位
    mstr_clear(&s);
...
void fmt_chrono_userdef_hour(void)
{
    ...
    // 24h: 1位~2位
    EVAL(mstr_format(
        "@{0:t:%H}@{1:t:%H}@", &s, 2, &time_value_sing, &time_value_doub
    ));
    ASSERT_EQUAL_STRING(&s, "@1@10@");
    // 24h: 固定2位
    mstr_clear(&s);
...
void fmt_chrono_userdef_second(void)
{
    ...
    // 1位~2位
    EVAL(mstr_format(
        "@{0:t:%s}@{1:t:%s}@", &s, 2, &time_value_sing, &time_value_doub
    ));
    ASSERT_EQUAL_STRING(&s, "@1@10@");
    // 固定2位
    mstr_clear(&s);
...
void fmt_chrono_userdef_subsecond(void)
{
    ...
    EVAL(mstr_create_empty(&s));
    // 1位
    EVAL(mstr_format("@{0:t:%x}@", &s, 1, &time_value));
    ASSERT_EQUAL_STRING(&s, "@1@");
    // 前2位
    mstr_clear(&s);
    EVAL(mstr_format("@{0:t:%xx}@", &s, 1, &time_value));
    ASSERT_EQUAL_STRING(&s, "@12@");
    // 前3位
    mstr_clear(&s);
    EVAL(mstr_format("@{0:t:%xxx}@", &s, 1, &time_value));
    ASSERT_EQUAL_STRING(&s, "@123@");
    // 前4位
    mstr_clear(&s);
    EVAL(mstr_format("@{0:t:%xxxx}@", &s, 1, &time_value));
...
void fmt_chrono_userdef_week(void)
{
    MString s;
    EVAL(mstr_create_empty(&s));
    EVAL(mstr_format("@{0:t:%w}@", &s, 1, &time_value));
